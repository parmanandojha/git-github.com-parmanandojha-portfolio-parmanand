...
var e = class {
	...
	_setInview(t) {
		const s = this.scrollElements.find((s) => s.$el === t.target);
		this.IORaf && s?.setInteractivityOn(), !this.IORaf && s?.setInview();
	}
	_setOutOfView(t) {
		const s = this.scrollElements.find((s) => s.$el === t.target);
		this.IORaf && s?.setInteractivityOff(), !this.IORaf && s?.setOutOfView(), null != s && s.attributes.scrollRepeat || this.IORaf || this.unobserve(t.target);
	}
};
...
var l = class {
	...
	onRender({ currentScroll: t, smooth: s }) {
		...
		if (this.currentScroll = t, this._computeProgress(), this.attributes.scrollSpeed && !isNaN(this.attributes.scrollSpeed)) if (this.attributes.scrollEnableTouchSpeed || s) {
			if (this.isInFold) this.translateValue = Math.max(0, this.progress) * e * this.attributes.scrollSpeed * -1;
			else this.translateValue = i(0, 1, -1, 1, this.progress) * e * this.attributes.scrollSpeed * -1;
	...
	_computeIntersection() {
		...
		if (this.intersection.end = b ? b(this.metrics.offsetStart, p, c) : this.metrics.offsetStart - p + c, this.intersection.end <= this.intersection.start) switch (f) {
			...
			case "middle":
				...
				break;
			case "end": this.intersection.end = this.intersection.start + c;
		}
	}
	_computeProgress(t) {
	...
	_setCustomEventProgress(t = 0) {
		...
		if (!s) return;
		const e = new CustomEvent(s, { detail: {
			target: this.$el,
			progress: t
		} });
...
var a = class {
	...
	onResize({ currentScroll: t }) {
		for (const s of this.RAFScrollElements) s.onResize({ currentScroll: t });
	}
	onRender({ currentScroll: t, smooth: s }) {
